...
import { useActiveAccount, useDisconnect, useActiveWallet, Blobbie } from "thirdweb/react";
import { 
  LogOut, Wallet, TrendingUp, TrendingDown, Activity, Settings, Copy, Check, 
  Shield, PieChart, ArrowUpRight, ArrowDownRight, DollarSign, Percent,
  Calendar, BarChart3, AlertCircle, ExternalLink, Loader2
} from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ client, fundId = 1 }) => {
  ...
  const ownershipPercent = (userData.total_shares / fundData.total_shares_outstanding) * 100;
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-2 bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-xl border border-amber-500/20 p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div>
                ...
                <p className="text-2xl font-bold text-white">{formatCurrency(fundData.total_aum)}</p>
              </div>
              <div>
                <p className="text-slate-400 text-sm mb-1">NAV/Share</p>
                <p className="text-2xl font-bold text-white">${formatNumber(fundData.current_nav_per_share)}</p>
              </div>
              <div>
                <p className="text-slate-400 text-sm mb-1">MTD Performance</p>
                <p className={`text-2xl font-bold ${fundData.performance_mtd >= 0 ? 'text-emerald-500' : 'text-red-500'} flex items-center gap-1`}>
                  {fundData.performance_mtd >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
                  {formatNumber(fundData.performance_mtd, 2)}%
                </p>
              </div>
              <div>
                <p className="text-slate-400 text-sm mb-1">YTD Performance</p>
                <p className={`text-2xl font-bold ${fundData.performance_ytd >= 0 ? 'text-emerald-500' : 'text-red-500'} flex items-center gap-1`}>
                  {fundData.performance_ytd >= 0 ? <TrendingUp className="w-5 h-5" /> : <TrendingDown className="w-5 h-5" />}
                  ...
                  {portfolioData.map((asset, index) => (
                    <div key={asset.asset_symbol} className="text-center">
                      <div className="flex items-center gap-1 justify-center mb-1">
                        ...
                        <p className="text-slate-400 text-xs">{asset.asset_symbol}</p>
                      </div>
                      <p className="text-white font-semibold text-sm">{formatNumber(asset.weight_percentage, 1)}%</p>
                    </div>
                  ))}
                </div>
              </>
        ...
        <div className="bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-xl border border-amber-500/20 p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
              <p className="text-slate-400 text-sm mb-1">Your Portfolio Value</p>
            ...
            <div>
              ...
              <p className={`text-2xl font-bold ${totalGainLoss >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                {formatCurrency(totalGainLoss)} ({formatNumber(totalGainLossPercent, 2)}%)
              </p>
            </div>
            <div>
              <p className="text-slate-400 text-sm mb-1">Your Shares</p>
              <p className="text-2xl font-bold text-white">{formatNumber(userData.total_shares, 8)}</p>
            </div>
            <div>
              <p className="text-slate-400 text-sm mb-1">Fund Ownership</p>
              <p className="text-2xl font-bold text-white">{formatNumber(ownershipPercent, 4)}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              {performanceData.length > 0 ? (
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={performanceData}>
                    ...
                    />
                    <Line type="monotone" dataKey="nav_per_share" stroke="#f59e0b" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              ) : (
            ...
            <div className="bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-xl border border-amber-500/20 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div className="bg-slate-800/50 rounded-lg p-4">
                  ...
                  <p className="text-xl font-bold text-white">{formatNumber(fundData.sharpe_ratio || 0, 2)}</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <p className="text-slate-400 text-sm mb-1">Max Drawdown</p>
                  <p className="text-xl font-bold text-red-500">{formatNumber(fundData.max_drawdown || 0, 2)}%</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <p className="text-slate-400 text-sm mb-1">Volatility</p>
                  <p className="text-xl font-bold text-white">{formatNumber(fundData.volatility || 0, 2)}%</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <p className="text-slate-400 text-sm mb-1">All-Time High</p>
                  <p className="text-xl font-bold text-emerald-500">${formatNumber(fundData.all_time_high_nav || 0)}</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <p className="text-slate-400 text-sm mb-1">All-Time Low</p>
                  <p className="text-xl font-bold text-slate-400">${formatNumber(fundData.all_time_low_nav || 0)}</p>
                </div>
                <div className="bg-slate-800/50 rounded-lg p-4">
                  <p className="text-slate-400 text-sm mb-1">Cost Basis</p>
                  <p className="text-xl font-bold text-white">${formatNumber(userData.cost_basis / userData.total_shares)}</p>
                </div>
              </div>
            </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="text-left py-3 px-2 text-slate-400 text-sm font-semibold">Asset</th>
                      <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">Quantity</th>
                      <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">Price</th>
                      <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">Value</th>
                      <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">24h</th>
                      <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">P&L</th>
                    </tr>
                  </thead>
                  <tbody>
                    {portfolioData.length > 0 ? (
                      portfolioData.map((asset) => (
                        <tr key={asset.asset_symbol} className="border-b border-slate-800 hover:bg-slate-800/30">
                          <td className="py-3 px-2">
                            <div className="flex items-center gap-2">
                              <div className="w-8 h-8 rounded-full bg-amber-500/10 flex items-center justify-center">
                                <span className="text-amber-500 font-bold text-xs">{asset.asset_symbol}</span>
                              </div>
                              <span className="text-white font-semibold">{asset.asset_name}</span>
                            </div>
                          </td>
                          <td className="text-right py-3 px-2 text-white">
                            {formatNumber(asset.quantity, asset.asset_symbol === 'USD' ? 2 : 4)}
                          </td>
                          <td className="text-right py-3 px-2 text-white">${formatNumber(asset.current_price)}</td>
                          <td className="text-right py-3 px-2 text-white font-semibold">{formatCurrency(asset.current_value)}</td>
                          <td className="text-right py-3 px-2">
                            <span className={`${asset.price_change_24h >= 0 ? 'text-emerald-500' : 'text-red-500'} font-semibold`}>
                              {asset.price_change_24h >= 0 ? '+' : ''}{formatNumber(asset.price_change_24h || 0, 2)}%
                            </span>
                          </td>
                          <td className="text-right py-3 px-2">
                            <span className={`${asset.unrealized_pnl_percentage >= 0 ? 'text-emerald-500' : 'text-red-500'} font-semibold`}>
                              {asset.unrealized_pnl_percentage >= 0 ? '+' : ''}{formatNumber(asset.unrealized_pnl_percentage || 0, 2)}%
                            </span>
                          </td>
                        </tr>
                      ))
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-xl border border-amber-500/20 p-6">
              ...
              {/* Mint Shares */}
              <div className="mb-6">
                <label className="text-slate-400 text-sm mb-2 block">Mint New Shares</label>
                <input
                  type="number"
                ...
                {mintAmount && (
                  <p className="text-slate-400 text-sm mb-2">
                    You will receive: {formatNumber(parseFloat(mintAmount) / fundData.current_nav_per_share, 4)} shares
                  </p>
                )}
              ...
              {/* Redeem Shares */}
              <div>
                <label className="text-slate-400 text-sm mb-2 block">Redeem Shares</label>
                <input
                  type="number"
                ...
                {redeemAmount && (
                  <p className="text-slate-400 text-sm mb-2">
                    You will receive: {formatCurrency(parseFloat(redeemAmount) * fundData.current_nav_per_share)}
                  </p>
                )}
            ...
            <div className="bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-xl border border-amber-500/20 p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  ...
                  <span className="text-white font-semibold">{formatNumber(userData.total_shares, 8)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  <span className="text-slate-400 text-sm">Current NAV</span>
                  <span className="text-white font-semibold">${formatNumber(fundData.current_nav_per_share)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  <span className="text-slate-400 text-sm">Investment Value</span>
                  <span className="text-white font-semibold">{formatCurrency(currentValue)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  <span className="text-slate-400 text-sm">Cost Basis</span>
                  <span className="text-white font-semibold">{formatCurrency(userData.cost_basis)}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  <span className="text-slate-400 text-sm">Unrealized P&L</span>
                  <span className={`font-semibold ${totalGainLoss >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                    {formatCurrency(totalGainLoss)}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-700/50">
                  <span className="text-slate-400 text-sm">P&L Percentage</span>
                  <span className={`font-semibold ${totalGainLossPercent >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                    {formatNumber(totalGainLossPercent, 2)}%
                  </span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-slate-400 text-sm">Fund Ownership</span>
                  <span className="text-white font-semibold">{formatNumber(ownershipPercent, 4)}%</span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                {marketData.length > 0 ? (
                  marketData.map((asset) => (
                    <div key={asset.asset_symbol} className="bg-slate-800/50 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-white font-bold">{asset.asset_symbol}</span>
                        <span className={`text-sm font-semibold ${asset.price_change_24h >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                          {asset.price_change_24h >= 0 ? '+' : ''}{formatNumber(asset.price_change_24h || 0, 2)}%
                        </span>
                      </div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-slate-400 text-xs">Price</span>
                        <span className="text-white font-semibold">${formatNumber(asset.current_price)}</span>
                      </div>
                      {asset.volume_24h && (
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-slate-400 text-xs">24h Volume</span>
                          <span className="text-slate-300 text-sm">${(asset.volume_24h / 1e9).toFixed(2)}B</span>
                        </div>
                      )}
                      {asset.market_cap && (
                        <div className="flex items-center justify-between">
                          <span className="text-slate-400 text-xs">Market Cap</span>
                          <span className="text-slate-300 text-sm">${(asset.market_cap / 1e9).toFixed(2)}B</span>
                        </div>
                      )}
                    </div>
                  ))
        ...
        <div className="bg-slate-900/90 backdrop-blur-xl rounded-xl shadow-xl border border-amber-500/20 p-6 mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700">
                  <th className="text-left py-3 px-2 text-slate-400 text-sm font-semibold">Date</th>
                  <th className="text-left py-3 px-2 text-slate-400 text-sm font-semibold">Type</th>
                  <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">Shares</th>
                  <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">Price</th>
                  <th className="text-right py-3 px-2 text-slate-400 text-sm font-semibold">Total Value</th>
                  <th className="text-center py-3 px-2 text-slate-400 text-sm font-semibold">Status</th>
                  <th className="text-center py-3 px-2 text-slate-400 text-sm font-semibold">Tx Hash</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={index} className="border-b border-slate-800 hover:bg-slate-800/30">
                      ...
                      <td className="py-3 px-2">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          tx.transaction_type === 'MINT' ? 'bg-emerald-500/20 text-emerald-500' : 
                          tx.transaction_type === 'REDEEM' ? 'bg-red-500/20 text-red-500' :
                          'bg-blue-500/20 text-blue-500'
                        }`}>
                          {tx.transaction_type}
                        </span>
                      </td>
                      <td className="text-right py-3 px-2 text-white">{formatNumber(tx.share_quantity, 2)}</td>
                      <td className="text-right py-3 px-2 text-white">${formatNumber(tx.share_price)}</td>
                      <td className="text-right py-3 px-2 text-white font-semibold">{formatCurrency(tx.total_usd_value)}</td>
                      <td className="text-center py-3 px-2">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          tx.status === 'CONFIRMED' ? 'bg-emerald-500/20 text-emerald-500' : 
                      ...
                      <td className="text-center py-3 px-2">
                        {tx.transaction_hash ? (
                          <a href={`https://etherscan.io/tx/${tx.transaction_hash}`} target="_blank" rel="noopener noreferrer" className="text-amber-500 hover:text-amber-400 flex items-center justify-center gap-1">
                            <span className="font-mono text-sm">{tx.transaction_hash.slice(0, 10)}...</span>
          ...
          <div className="space-y-3">
            {activities.length > 0 ? (
              activities.map((activity, index) => (
                <div key={index} className="bg-slate-800/50 rounded-lg p-4 border border-slate-700 hover:border-amber-500/30 transition-all">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${
                          activity.activity_type === 'TRADE' ? 'bg-blue-500/20 text-blue-400' :
                          activity.activity_type === 'REBALANCE' ? 'bg-purple-500/20 text-purple-400' :
                          activity.activity_type === 'DISTRIBUTION' ? 'bg-emerald-500/20 text-emerald-400' :
